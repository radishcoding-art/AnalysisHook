...
#   (b) 自证段正文 >= 100 字符, 含 结论 / 依据 / 自我检查 三段 (require_reviewer_call=true 时还要 独立审查段)
#   (c) require_reviewer_call=true 时, 本回合 transcript 含 Agent 调用 + subagent_type 在 allowed_reviewer_subagents 白名单
# 任一不满足 → block, AI 必须补齐. 连续 block 3 次后降级 warning (P0-3 修复防死循环)
# 当前 require_reviewer_call=false: 关闭外部 reviewer 强制 (用户偏好节省 token), 只要求 AI 自证三段.
conclusion_self_review_check:
  ...
  required_self_review_sections:
    ...
    - "## 自证与审查"
    - "## Self-Review"
  # 是否强制调用外部 reviewer subagent.
  # false  = 只要求 AI 在自证段写 结论 / 依据 / 自我检查 三段, 不要求外部 reviewer (用户偏好, 节省 token)
  # true   = 同时强制本回合 transcript 含 allowed_reviewer_subagents 之一的 Agent 调用
  require_reviewer_call: false
  # 合法 reviewer subagent 白名单 (require_reviewer_call=true 时生效, 精确匹配, P0-4 修复)
  allowed_reviewer_subagents:
    - "superpowers:code-reviewer"
    - "code-reviewer"

# Stop hook 第 6 层: 计算强制验证
# 触发: 回复中含数字运算痕迹 (算术表达式 / 结果声明词), 但本回合未调用 python
# 要求: 任何数字运算 (加减乘除 / 位运算 / 地址偏移 / 字节长度 / 进制转换 / 百分比 / 时间换算)
#       必须用 `Bash(python -c "...")` 执行, 把 python 输出贴回回复, 再做结论
# 豁免: 代码块 (``` ... ```), 行内 code (` ... `), 引用行 (> 开头) 自动跳过
# 配置可关: enabled=false 即跳过整个 L6
arithmetic_verification_check:
  enabled: true
  # 算术表达式正则: 明确含运算符的模式 (避免单纯数字误判)
  arithmetic_patterns:
    # 十进制算术: 5 + 3 = / 100 * 2 =
    - "\\d+\\s*[+\\-*/]\\s*\\d+\\s*="
    # 十六进制算术: 0x1000 + 0x20 / 0x7ff6... - 0x400
    - "0x[0-9a-fA-F]+\\s*[+\\-*/]\\s*(?:0x[0-9a-fA-F]+|\\d+)"
    # 混合: 4096 + 0x20
    - "\\d+\\s*[+\\-*/]\\s*0x[0-9a-fA-F]+"
  # 结果声明词: 隐含已经做了计算的句式
  result_claim_triggers:
    - "计算得"
    ...
    - "约\\s*(?:为\\s*)?\\d+"
    - "大约\\s*\\d+"
  # 合法的 python 执行模式 (Bash command 含其一即视为已验证)
  python_bash_patterns:
    - "\\bpython\\b"
    - "\\bpython3\\b"

# Stop hook 第 7 层: 禁止未证伪就切换方向
# 触发: 回复中含 "切换意图" 短语 (改走 / 换思路 / 放弃当前 / 像 F<id> 一样 / ...)
# 要求 (任一满足即放行):
#   (a) 本回合写入 dead_ends.md (登记当前路径为死胡同, 证据机制)
#   (b) 回复含 "## 方案切换评估" section, 正文 >= 100 字符 + 至少 1 个 F<id> cite
#   (c) 回复含 "user-told:" cite (用户授权切换)
# 豁免: 代码块 / 行内 code / 引用行 (> 开头) 自动跳过, 和 L6 一致
approach_switch_check:
  enabled: true
  # 切换意图短语 (Python regex)
  trigger_phrases:
    - "改走"
  ...
  exemption_prefixes:
    ...
    - "备选"
    - "权衡"
  # dead_ends 文件路径 (相对项目根). check_l7 扫本回合 Edit/Write/MultiEdit tool_use 目标是否命中
  dead_ends_path: ".claude/state/dead_ends.md"
  # 方案切换评估 section header (line-anchored)
  required_section_headers:
    - "## 方案切换评估"
    - "## Approach Switch"
  # 评估 section 最小正文长度 (字符)
  min_section_body_length: 100

# Stop hook 第 8 层: 禁止 AI 自发停止
# 触发: 回复中含 "自发停止" 短语 (今天先到这 / 暂停 / 休息 / 改天再 / ...)
# 通过条件 (任一满足):
#   (a) plan.md frontmatter status == "completed" (任务真的完成)
#   (b) plan.md frontmatter paused == true (用户已通过 /pauseAnalysis 标记)
#   (c) 本回合 user message 含暂停指示词 (用户主动让 AI 停)
#   (d) 回复含 user-told: cite 引用用户的暂停指令
# 豁免: 等待用户操作的 handoff 句式 ("等你确认 / 等触发 / 请去做 X 后告诉我") 不算停止
# 配置可关: enabled=false 即跳过 L8
self_stop_check:
  enabled: true
  # 自发停止短语 (Python regex)
  trigger_phrases:
    ...
    - "(等下次|等下回|等后续|下次|改日)再"
    - "(收工|下班|打烊)"
  # 暂停指示词 (用户的 message 含其一 → 视为用户主动暂停, 通过)
  user_pause_phrases:
    - "休息"
    - "暂停"
    - "停一下"
    ...
    - "/pauseanalysis"
    - "/pauseAnalysis"
  # Handoff 豁免短语 (回复中含其一 → 视为合法等待, 不算自发停止)
  exemption_phrases:
    - "等你"
    - "等用户"
    - "等触发"
...
pretool_intercepts:
  ...
  # Plan 切换守卫: 拦截静默切换 current_step (配合 Stop hook L7 双层防御)
  - check: check_plan_switch_evidence
    tool_pattern: "Edit|Write|MultiEdit"
    file: ".claude/state/plan.md"
    rule: "如果 post-edit 的 current_step != pre-edit 的 current_step 且 pre 非空, 那么 post 内容里 pre 对应的 step status 必须是 completed 或 failed. 否则 block."
    reason: "防止 AI 不收尾就跳过未完成的 step (静默切换方案). 配合 Stop hook L7 拦截文字层切换声明, 两层共同覆盖. 合法操作: 用一次 MultiEdit 原子地改旧 step status + 切 current_step."
...
3. **频繁换方案**: 一个方向还没彻底走完就放弃换. 必须把当前 step 走到 plan 里写明的验证条件, 才能 advance 到下一个 step. **双层防御**:
   - **PreToolUse plan 切换守卫**: Edit/Write/MultiEdit 改 `plan.md` 时, 如果 `current_step` 从 A 变 B (A 非空 A≠B), post-edit 内容里 A 的 status 必须是 `completed` 或 `failed`, 否则 block. 合法操作: 用一次 `MultiEdit` **原子地**同时改旧 step status + 切 current_step (hook 只看 post 状态).
   - **Stop hook L7 (approach_switch_check)**: 回复文字含 "改走 / 换思路 / 另辟 / 放弃当前 / 重新规划 / 像 F<id> 一样 / ..." 等切换意图, 必须满足任一证据条件: (a) 本回合 Edit/Write 了 `dead_ends.md`, (b) 回复含 `## 方案切换评估` 段 (>= 100 字符 + F<id> cite), (c) `user-told:` cite. 否则 block. 豁免: 头脑风暴 / 讨论 / 考虑 / 比较 / 权衡 / 备选 前缀, 代码块, `>` 引用行.
...
**核心原则**: AI 得出任何**结论** (不只是事实, 而是从多个 fact **推导**出的判断) 之前, 必须先自证. 这是防止 "上下文偏见 / sunk cost / 逻辑漏洞" 的最后防线.

**注**: 当前 frontmatter `conclusion_self_review_check.require_reviewer_call: false`, 即**不要求**调用外部 reviewer subagent. AI 只需在回复中写自证段 (结论 / 依据 / 自我检查 三段) 即可. 想恢复外部审查: 把该字段改成 true.
...
1. **自证段 header**: 回复中含 `## 自证` 或 `## 自证与审查` 或 `## Self-Review` (完整 markdown header)
2. **自证段正文 ≥ 100 字符**, 且含三个子段:
   - **结论** (一句话复述)
   - **依据 (facts)** (cite F<id>)
   - **自我检查** (回答 4 个问题: 上下文偏见 / sunk cost / 逻辑漏洞 / 确认偏见)
